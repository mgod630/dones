...
class Courses_news:
    ...
    @staticmethod
    def insert_new_course_news(section_id, jalali_date, course_news_type, course_news_text):
        global connection_pool
        if connection_pool == None:
            connection_pool = app.config['mysql_connection_pool']
        cnx = connection_pool.get_connection()
        cursor = cnx.cursor(dictionary=True)
        add_course_news = ("INSERT INTO `tbl_courses_news` (`section_id`, `jalali_date`, `course_news_type`, `course_news_text`) VALUES" +
                      "( %(section_id)s, %(jalali_date)s, %(course_news_type)s, %(course_news_text)s)")
        data_course_news = {
    ...
    @staticmethod
    def update_course_news(id, section_id, jalali_date, course_news_type, course_news_text):
        global connection_pool
        if connection_pool == None:
            connection_pool = app.config['mysql_connection_pool']
        cnx = connection_pool.get_connection()
        ...
        if course_news_type:
            update_string += f'course_news_type=%(course_news_type)s,'
        if course_news_text:
