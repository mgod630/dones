...
def make_routes(goldis_blueprint):
    ...
    def json_get_comments_and_courses_news_by_section_id3(item_id):
        ...
        a_page_section_comments = comments_orm.Comments.get_comments_by_section_id(item_id, comments_count_per_page= comments_count_per_page, reversed_ordering= True)
        start = page_number * comments_count_per_page - comments_count_per_page if page_number > 0 else 0
        end = start + comments_count_per_page 
        a_page_section_comments = a_page_section_comments[start:end]
